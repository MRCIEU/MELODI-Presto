...
def enrich(q):
    ...
    try:
        res = response.json()
        enrich_df = pd.json_normalize(res)
        return enrich_df
    except:
        print("No data")
        return []
...
def overlap(q1, q2):
    ...
    res = response.json()
    if "data" in res:
        overlap_df = pd.json_normalize(res["data"])
    else:
...
def sentence(q):
    ...
    response = requests.post(url, data=json.dumps(params))
    res = response.json()
    pub_df = pd.json_normalize(res["data"])
    return pub_df
...
def plot_overlap_counts(overlap_counts):
    ...
    overlap_counts.object_name_x = (
        overlap_counts.object_name_x + " (" + overlap_counts.object_type_x + ")"
    )
...
def create_overlap_network(overlap_df):
    ...
    overlap_df.loc[:, "set_x"] = overlap_df["set_x"].apply(lambda x: x.upper())
    overlap_df.loc[:, "set_y"] = overlap_df["set_y"].apply(lambda x: x.upper())
